...
object FindFixVerifyTranslation extends App {
	val textToImprove =
		"""Google developed nanoparticles that are to detect signs of cancer or other disease
		  ...
		  | early warning system for life-threatening diseases."
		""".stripMargin //source: google translate of first paragraph from http://www.heise.de/newsticker/meldung/Google-will-Nanopartikel-nach-Krebszellen-suchen-lassen-2438107.html

	val patches = textToImprove.split(".").map(_.trim).toList

	HComp.addPortal(new CrowdFlowerPortalAdapter("Fix-Up English Translation"))
	...
	println(ffv.bestPatches.mkString("."))
}
