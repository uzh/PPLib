...
import ch.uzh.ifi.pdeboer.pplib.hcomp.{StringQuestionRenderer, HComp}
...
object DualPathwayTranslation extends App {
	...
	val process = new DualPathwayProcess(Map(
		...
		QUESTION_OLD_PROCESSED_ELEMENT.key -> StringQuestionRenderer("Please fix grammatical mistakes and spelling errors in the following paragraph by rewriting it into the text box below.", "Other Crowd workers corrected the text above and turned it into the text below. Please add any corrections you'd like to make"),
		QUESTION_NEW_PROCESSED_ELEMENT.key -> StringQuestionRenderer("Please fix grammatical mistakes and spelling errors in the following paragraph by rewriting it into the text box below.")
	))
