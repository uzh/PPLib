...
import ch.uzh.ifi.pdeboer.pplib.hcomp.{StringQuestionRenderer, HCompPortalAdapter, MockHCompPortal}
...
class ContestWithMultipleEqualWinnersTest {
	...
	def testPartitioning: Unit = {
		...
		val finder = new ContestWithMultipleEqualWinnersPublic(data, StringQuestionRenderer(""), "title", 2,
	...
	def testSelection: Unit = {
		...
		val finder = new ContestWithMultipleEqualWinnersPublic(data, StringQuestionRenderer("nom"), "title", 2,
	...
	private class ContestWithMultipleEqualWinnersPublic(data: List[Patch], question: StringQuestionRenderer, title: String, findersPerItem: Int, shuffle: Boolean, portal: HCompPortalAdapter, maxItemsPerFind: Int = 5, memoizer: ProcessMemoizer = new NoProcessMemoizer())
