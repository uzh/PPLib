...
import ch.uzh.ifi.pdeboer.pplib.process.recombination.{ProcessSurfaceStructure, RecombinationVariantProcessXMLExporter}
...
class ProcessCandidateProcessXMLExporterTest {
	...
	def testStringListExport: Unit = {
		...
		val variant = new ProcessSurfaceStructure(Map("testprocess" -> processType))
