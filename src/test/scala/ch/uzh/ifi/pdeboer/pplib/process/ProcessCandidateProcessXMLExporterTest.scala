...
import ch.uzh.ifi.pdeboer.pplib.process.recombination.{ProcessDeepStructure, RecombinationVariantProcessXMLExporter}
...
class ProcessCandidateProcessXMLExporterTest {
	...
	def testStringListExport: Unit = {
		...
		val variant = new ProcessDeepStructure(Map("testprocess" -> processType))
