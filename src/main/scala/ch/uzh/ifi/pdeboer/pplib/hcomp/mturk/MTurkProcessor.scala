...
class MTurkManager(val service: MTurkService, val query: HCompQuery, val properties: HCompQueryProperties) extends LazyLogger {
	...
	def createHIT(): String = {
		...
		val hitTypeID = service.RegisterHITType(query.title.take(120), query.question, Price(dollars.toString), ONE_DAY, Seq.empty[String], ONE_DAY, Seq.empty[QualificationRequirement])
