...
object FeatureInfluenceOnIncome_Experiment1 extends App with LazyLogger {
	val features = CSVReader.open("example_data/featuresWithInfluenceOnIncome2.csv").all().map(l => Feature(l.head)(l(1)))
	...
	def getEstimationForFeature(feature: Feature) = {
		val instructions = new TrivialInstructionGenerator("What do you think about the following: In the USA, ",
			"Please estimate the influence of this economic factor", questionAfter = "You can accept multiple of these HITs, but please only *one per variable* (marked with the asterisks **)")
