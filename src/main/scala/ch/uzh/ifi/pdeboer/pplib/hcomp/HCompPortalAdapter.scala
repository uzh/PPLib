...
trait AnswerRejection {
	...
	def rejectAnswer(a: HCompAnswer, message: String = ""): Boolean = {
		synchronized {
			val a = unapprovedAnswers.remove(a)
			a.map(_.reject(message)).getOrElse(false)
		}
	}

	def approveAndBonusAnswer(a: HCompAnswer, message: String = "", bonusCents: Int = 0): Boolean = {
		synchronized {
			val a = unapprovedAnswers.remove(a)
