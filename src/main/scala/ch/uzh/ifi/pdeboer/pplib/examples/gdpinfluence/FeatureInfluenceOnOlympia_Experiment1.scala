...
object FeatureInfluenceOnOlympia_Experiment1 extends App with LazyLogger {
	...
	def getEstimationForFeatureThroughCollection(feature: Feature) = {
		val instructions = new TrivialInstructionGenerator("What do you think about the following: ",
			"How well can you predict the olympics?", questionAfter = "Your answer must be a number. Characters are NOT allowed. Please only accept one of these HITs. ") //You can accept multiple of these HITs, but please only *one per topic* (topics marked with the asterisks **).
